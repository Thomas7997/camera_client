...
unsigned int get_files_and_dirs (char *** dirs_b, char ** lines, unsigned int nb, unsigned int * sizes_list, char ** files, char ** dirs) {
    // Executer read_dir_list, read_file_list puis blockerize ...
    unsigned int * refs = calloc(100, sizeof(unsigned int));
    unsigned int x, y, z;
    char * path = calloc(100, sizeof(char));
    strcpy(path, "files.txt");

    printf ("Parsing ...\n");

    printf("1");

    unsigned int lines_nb = load_dossiers (dirs, path, refs);
    unsigned int tmp_size, tmp_y = 0;

    printf ("1");

    for (x = 0; x < lines_nb; x++) {
        tmp_size = read_file_list(files, lines, nb, refs[x], z); // refs est les lignes où commencer
        for (y = 0; y < tmp_size; y++) {
            strcpy(dirs_b[x][y], "");
            sprintf(dirs_b[x][y], "%s/%s", dirs[x], files[tmp_size+y]);
        }
        tmp_y += tmp_size;
        z += tmp_size-1;
    }
...
int main (void) {
    ...
    char ** files = calloc(MAX_CAPTURES, sizeof(char*));
    char ** dirs_l = calloc(MAX_DIRS, sizeof(char*));
    ...
    for (int s = 0; s < MAX_DIRS; s++) {
        dirs_l[s] = calloc(100, sizeof(char));
        dirs[s] = calloc(MAX_CAPTURES, sizeof(char*));
        for (int z = 0; z < MAX_CAPTURES; z++) {
            dirs[s][z] = calloc(TAILLE_NOM, sizeof(char));
        }
    }
    ...
    unsigned int files_nb = get_files_and_dirs(dirs, liste_captures, number, dir_sizes, files, dirs_l);
